...
class ParseModel:
    def __init__(
        ...
        self._deprels = tf.placeholder(tf.int32, [batch_size, n_deprels], "deprels")

        n_features = int(shapes['features'])
        self._features = tf.placeholder(tf.int32, [batch_size, n_features], "features")
        ...
        self._token_embeds = tf.placeholder(tf.float32, [None, 50], "token_embeds")
        ...
        with tf.variable_scope("input_norm"):
            inputs = tf.layers.batch_normalization(inputs, scale = True, momentum=0.98, training = self.is_training, fused=True)

        if config.keep_prob_input < 1:
            inputs = tf.contrib.layers.dropout(inputs, keep_prob = config.keep_prob_input, is_training = self.is_training)
        ...
        with tf.variable_scope("hidden_norm"):
            hidden = tf.layers.batch_normalization(hidden, scale = True, momentum = 0.97, training = self.is_training, fused=True)

        if config.keep_prob < 1:
            hidden = tf.contrib.layers.dropout(hidden, keep_prob = config.keep_prob, is_training = self.is_training)
